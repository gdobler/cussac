...
def loadComplaints(complaintsFilename):
	...
	with open(complaintsFilename) as f:
		...
		zipIndex = headers.index('Zip Code')
...
def drawPlot(shapeFilename, mapPoints, zipBorough, mapPoints1):
	# Read the ShapeFile
	dat = shapefile.Reader(shapeFilename)
  
	# Creates a dictionary for zip: {lat_list: [], lng_list: []}.
	zipCodes = []
	polygons = {'lat_list': [], 'lng_list': [], 'color_list' : [],'radius_list1' : [], 'centerlat_list' : [] ,'centerlng_list' : [], 'radius_list' : []}
	...
	sortedlist = sorted(mapPoints1['zip_complaints'].items(), key=operator.itemgetter(1), reverse=True)
	complaintsHighestTotal1 = sortedlist[0][1]
	...
	for r in dat.iterRecords():
		...
		if currentZip in zipBorough:
			...
			if currentZip in mapPoints1['zip_complaints']:
				compCount = mapPoints1['zip_complaints'][currentZip]
				percentage = float(compCount)/float(complaintsHighestTotal1)
				
				relRadius1 = (maxRadius-minRadius)*percentage
			...
			polygons['radius_list1'].append(relRadius1)
		record_index += 1



	# Creates the Plot
	titleString = "New York Through the Instagram Lens\n 12-30-14 to 1-4-15"
	
	output_file("NYC Plot 12-30 to 1-4.html", title=titleString)
	...
	circle(polygons['centerlng_list'], polygons['centerlat_list'], radius=polygons['radius_list1'], 
		fill_color='yellow', fill_alpha=0.4, line_color=None)
...
if __name__ == '__main__':
	...
	complaintsFile1 = sys.argv[4]
	mapPoints1 = loadComplaints(complaintsFile1)
	mapPoints = loadComplaints(complaintsFile)
	zipBorough = getZipBorough(zipBoroughsFile)
	drawPlot(shapeFile, mapPoints, zipBorough, mapPoints1)
