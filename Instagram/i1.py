...
#Creating the csv file at a directory called output in
#the current working directory which stores all the csv files
#stored as 1.csv, 2.csv, etc.
num_of_file = 1
name_of_file = str(num_of_file)
getwd = os.getcwd()
path = getwd + "/output"
if not os.path.exists(path):
	os.makedirs(path)
name_of_file = name_of_file + ".csv"
f = open(os.path.join(path, name_of_file),'w')
f.write('')
f.close()
...
f = open(os.path.join(path, name_of_file), 'a+')
...
while True:
	...
	try:
		...
		try:
			...
			num_of_file += 1
			name_of_file = str(num_of_file) + ".csv"
			f.close()
			f = open(os.path.join(path, name_of_file), 'w')
			insta_write = csv.writer(f, delimiter = ',')
			insta_write.writerow(header)
