	...
	try:
		...
		for zipcode, lt, lon in latlngs:
			...
			for media in area:
				...
				#print proj(temp_lat, temp_lng)
				checklat, checklng = proj(temp_lat,temp_lng)
				print proj(checklat,checklng, inverse = True)
				for x,i in boros.iterrows():
					#print i.geometry.contains(Point(temp_lat,temp_lng))
					if i.geometry.contains(Point(checklat,checklng)): 
