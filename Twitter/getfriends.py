...
#initialize the call with -1
    ...
    if not os.path.isfile('followingids/'+ username + 'followerids.csv'):
        df.to_csv('followingids/'+ username + 'followerids.csv')
    while cursor !='0':
        ...
        try:
            ...
            with open(os.readlink(output)+ username + 'followerids.csv', 'a') as f:
...
def main():
    ...
    #username = sys.argv[1]
    for file_name in os.listdir(os.readlink(output)):
        users = pd.read_csv(os.readlink(output) + str(file_name), index_col = 0)
